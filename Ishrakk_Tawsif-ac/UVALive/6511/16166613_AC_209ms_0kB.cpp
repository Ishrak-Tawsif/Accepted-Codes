...
int ar[100005],vis[100005],iscycle[100005],cycle;
...
void dfs(int cur,int id)
{
    ...
    if(!cycle  && vis[now] != id && !iscycle[vis[now]])dfs(now,id);
}
...
int main()
{
    ...
    for(int tt=1; tt<=tc; tt++)
    {
        ...
        {
            int nowid = vis[i];
            if(!iscycle[nowid]) cycle = 0,dfs(i,i);
            //cout<<i<<" .... " << cycle<<endl;
        }
        ...
        for(int i=1; i<=n; i++)
        {
            int nowid = vis[i];//cout<<i<<" "<<nowid<<endl;
            if(!iscycle[nowid]) res++;
        }
        printf("%d\n", res);
    }
