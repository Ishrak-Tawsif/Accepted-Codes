...
{//cout<<ind<<" .. "<<curm<<endl;
    ...
    //ans2 = max(ans,call(ind+1,curm));
    //ans = max(ans,call(ind+2,curm));

    for(int i=ind; i<=n; i++)
    {
        cnt++;
        sum = sum + (ar[i]*(ll)cnt);//if(ind == 1)cout<<cnt<<" "<<sum<<endl;
        if(curm + cnt <= m)
        {
            if(cnt <= r)
            {//if(ind == 1)cout<<" sum : "<<sum<<" "<<curm+cnt<<endl;
                ...
                //,call(ind+cnt+1,curm+cnt)+(sum-(ar[i]*(ll)cnt)));
            }
//            else
//            {
//                for(int j=1; ; j++)
...
//                }
//            }
            //else ans2 = max(ans2,call(ind+cnt,curm));
        }
        //ans = max(ans,)
    }//cout<<ind<<" "<<curm<<" "<<ans<<endl;
