...
{//READ("in.txt");
    ...
    while(tc--)
    {
        ...
        for(int i=0; i<26; i++)
        {
            if(cnt[i] > maxi) maxi = cnt[i],ind = i;
        }

        int koyta = 0;
        for(int i=0; i<26; i++) if(cnt[i] == maxi) koyta ++;
...
//cout<<ind<<" ... " << dis<<endl;
        for(int i=0; i<len; i++)
        {
            ...
            now -= dis;
            now += 26;
            now %= 26;//cout<<now<<endl;
            //if(now == 0) now = 27;
            str[i] = temp[now];
        }

        if(dis<0) dis = abs(dis);
        else if(dis>0) dis = abs(dis);
