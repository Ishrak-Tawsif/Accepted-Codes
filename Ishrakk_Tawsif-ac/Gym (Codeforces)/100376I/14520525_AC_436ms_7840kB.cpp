...
ll call(int n,ll a)
{
    ll maxi = -2e9, curmaxi = 0;

    for (int i = 0; i < n; i++)
    {
        curmaxi = curmaxi + ar[i];//out(curmaxi*a);
        if (maxi < curmaxi*a) maxi = curmaxi*a;

        if (curmaxi*a < 0) curmaxi = 0;
    }
    return maxi;
}
...
int main()
{
    ...
    for(int i=0; i<n; i++) sfll(ar[i]),max(mxx,ar[i]);
