...
//        {
//            dfs(now);
//            cost[indd].cst = graph[cur][i].second;
//            cost[indd].ind = now;
//            cost[indd].cst *= totdep[now];
//            totdep[cur] += dep[now];
//            indd++;
//        }
//    }
//    //cost[cur].cst *= (totdep[cur]);//cout<<cur<<" "<<totdep[cur]<<" "<<cost[cur].cst<<endl;
//}
...
ll dfs(int cur,int par)
{
    ...
    for(int i=0; i<graph[cur].size(); i++)
    {
        ...
        if(v != par)
        {
            now = dfs(v,cur);//out(now * graph[cur][i].second);
            vv.pb(now * graph[cur][i].second);
            totpep += now;
        }
    }
    return totpep;
...
int main()
{
    ...
    for(int i=1; i<=n; i++)
    {
        ...
        ll cur = 0;//cout<<" size " <<vv.size()<<endl;
        for(int j=k; j<vv.size(); j++)
