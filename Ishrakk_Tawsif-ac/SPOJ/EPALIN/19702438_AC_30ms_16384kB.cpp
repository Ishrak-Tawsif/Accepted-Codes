...
int main()
{
    ...
    pw[0] = base;
    for(int i=1; i<100005; i++) pw[i] = pw[i-1] * base;

    while(cin>> str)
    {
        int len = str.length(),ind=0,last=len-1;
        ull f = get(str[len-1]),r = get(str[len-1]);//out(get(str[len-1]));
        for(int i=str.length()-2; i>=0; i--)
        {//out(get(str[i]));
            f = (f*base) + get(str[i]);
            r = (get(str[i]) * pw[ind++]) + r;

            if(f == r) last = i;
        }
        cout<<str;
        for(int i=last-1; i>=0; i--) cout<<str[i];
        cout<<endl;
    }
