...
ll checkprime[mx]; vector<ll>prime;
void sieve() { ll n,i,j; for(i=4; i<mx; i+=2) checkprime[i] = 1;  for(i=3; i*i<=mx; i+=2){ if(checkprime[i]==0){ for(j=i*i; j<mx; j+=(i+i)) checkprime[j] = 1; }  }
...
ll n,m,t,i,j=0,k,ck=0,dk=0,a,b,c,d,e,f;
ll dp[10005];
