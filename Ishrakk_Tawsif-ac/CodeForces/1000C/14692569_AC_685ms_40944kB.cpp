...
int main()
{
    ...
    ll now = 0;
    ll fs = *st.begin(); st.erase(st.begin());
    ll sn = *st.begin(); st.erase(st.begin());

    for(int i=1; i<ind; i++)
    {
        now += (cnt[i].first + cnt[i].second);
        ans[now] += (sn-fs);
        fs = sn;
        if(!st.empty()) {sn = *st.begin();st.erase(st.begin());}
    }
