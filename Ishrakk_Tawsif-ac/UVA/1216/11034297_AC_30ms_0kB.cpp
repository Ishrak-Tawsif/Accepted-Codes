...
//cout<<in << " in " <<endl;
        for(int i=0; i<vv.size(); i++)
        {
            ...
            for(int j=i+1; j<vv.size(); j++)
            {
                ...
                //cout<<curu << " " << tempu << " " << curv << " " << tempv << endl;
                double dis = sqrt(((curu-tempu)*(curu-tempu)) + ((curv-tempv)*(curv-tempv)));
                get.u = i;
                get.v = j;
                get.cost = (ll) ceil(dis);//cout<<i << " " << j <<" " << get.cost << endl;
                best.pb(get);
                //worse.pb(get);
            }
        }
        ...
        /*ll kk = 0,res;
        for(int i=ttmm.size()-1; i>=0; i--)
        {
            if(kk == n-1) res = ttmm[i],cout<<i<<endl;;
            kk++;
        }
cout<<ttmm.size()-n<<endl;*/
        pf("%lld\n", ttmm[ttmm.size()-n]);
    }
    return 0;
}