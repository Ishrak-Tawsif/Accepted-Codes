...
int call(int pos,int sum)
{
    ...
    //if(sum>5000) return -2e9;
    if(dp[pos][sum] != -1) return dp[pos][sum];

    int ans;

    ans = min(call(pos+1,sum+ar[pos]),call(pos+1,sum));
...
{//WRITE("out.txt");
    ...
    while(scanf("%d %d", &n,&x))
    {
        ...
        {
            scanf("%lf", &temp);
            ar[i] = (temp * 100.0) + eps;//out(ar[i]);
        }
        swap(ar[0],ar[x-1]);//for(int i=0; i<x; i++) out(ar[i]);
        mem(dp,-1);
        int ans = call(1,ar[0]);
//out(ans);
        double res = ((((ar[0] * 1.0 / (1.0 * ans))) ) * 100.00) + eps;
        //res = (res / 100) + eps;
