...
void update(int pos,int val)
{
    ...
    /*if(pos%blocksize != 0)
    {
        ...
        for(int i=posind; i<blocksize; i++) csum[posblock][i] += val;
    }*/

    int posind = pos%blocksize;
    //cout<<ar[stblock][posind]<< " " << val-ar[stblock][posind] <<endl;
...
//cout<<uind << " " << vind << endl;
        ...
        {
            ans += ar[stblock][i];//cout<<csum[stblock][i]<<" "<<i<<endl;
        }
    }
...
{//WRITE("in.txt");
    ...
    while(sf(n)==1 && n)
    {
        ...
        for(int i=0; i<n; i++)
        {
            ...
            sf(ar[ind][j]);//cout<<ind<<" " << j <<endl;
            if(j==0)csum[ind][j]=ar[ind][j];
            else csum[ind][j] = ar[ind][j];
            csum2[ind] += ar[ind][j];
            j++;
        }
//system("pause");
