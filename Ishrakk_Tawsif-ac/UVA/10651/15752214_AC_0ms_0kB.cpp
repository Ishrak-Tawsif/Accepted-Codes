...
int call(int mask)
{
    ...
    for(int i=0; i<len; i++)
    {//out(mask);
        if(!(mask & (1<<i))) continue;//out("ok");
        if(i-2>=0)
        {
            int p = i-1;
            int pp = i-2;

            if((mask & (1<<p)) && (!(mask & (1<<pp))))
            {//out("ok");
                int tempmask = mask;
                tempmask = bitOn(tempmask,pp);
                tempmask = bitOff(tempmask,p);
                tempmask = bitOff(tempmask,i);

                ans = max(ans,call(tempmask)+1);
            }
        }
        if(i+2<len)
        {//out("ok");
